...
@pytest.mark.parametrize(
    ...
    [
        (2, 3, 8),                # Basic positive integers
        (5, 0, 1),                # Any number to the zero power is 1
        (2, -1, 0.5),             # Negative exponent yields fractional result
        (2.5, 2, 6.25),           # Float base
        (-3, 3, -27),             # Negative base with odd exponent
    ],
    ids=[
...
def test_power(base: Number, exponent: Number, expected: Number) -> None:
    """Verify the power function raises a number to the provided exponent."""
