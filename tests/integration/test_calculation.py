...
def test_exponentiation_get_result():
    """Test that Exponentiation.get_result calculates the base raised to the exponent."""
    inputs = [3, 4]
    power_calc = Exponentiation(user_id=dummy_user_id(), inputs=inputs)
    result = power_calc.get_result()
...
def test_calculation_factory_power():
    """Test the Calculation.create factory for the new power operation."""
    inputs = [2, 5]
    calc = Calculation.create(
    ...
    assert calc.get_result() == 32, "Incorrect power result."
...
def test_invalid_inputs_for_exponentiation():
    """Verify that exponentiation requires exactly two inputs."""
    power_calc = Exponentiation(user_id=dummy_user_id(), inputs=[2])
    with pytest.raises(ValueError, match="base and exponent"):
        power_calc.get_result()
