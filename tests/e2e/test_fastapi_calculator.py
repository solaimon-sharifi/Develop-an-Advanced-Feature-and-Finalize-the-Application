...
def test_create_calculation_power(base_url: str):
    ...
    data = response.json()
    # Expected result: 2^5 = 32
    assert data.get("type") == "power", "Expected type to be power"
    assert "result" in data and data["result"] == 32, f"Expected result 32, got {data.get('result')}"
