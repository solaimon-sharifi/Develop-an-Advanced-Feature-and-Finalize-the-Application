...
class Exponentiation(Calculation):
    """Exponentiation calculation (base raised to exponent)."""

    __mapper_args__ = {"polymorphic_identity": "power"}

    def get_result(self) -> float:
        """Raise the first input (base) to the second (exponent)."""
        if not isinstance(self.inputs, list):
            raise ValueError("Inputs must be a list of numbers.")
        if len(self.inputs) != 2:
            raise ValueError("Power calculations require base and exponent values.")
